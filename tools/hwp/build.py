...
KIND = {
    'h2':  (17, 19), 'h3':  (17, 20), 'h3b': (27, 20),
    'sec': (23, 21), 'it':  (23, 17), 'sb':  (23, 17),
    'bl':  (23, 17), 't1':  (23, 19), 't2':  (17, 19), 't3': (23, 17),
}
...
for i, (tag, lvl, p, ho, po) in enumerate(recs):
    ...
    if tag == H.TAG + 1:                                   # ID_MAPPINGS : 글자모양 수 +1
        ...
        struct.pack_into('<i', body, 9 * 4, n + 1)
    ...
    if i == last_cs:
        out += red_rec                                     # 마지막 글자모양 바로 뒤에 끼운다
doc_new = bytes(out)
...
print('빨간 글자모양 번호 %d (글자모양 %d개 → %d개)' % (RED_ID, len(cs_idx), len(cs_idx) + 1))
