...
- **홈**에서만 상단 헤더(로고 + 탭줄)를 보여 준다.
- **사이트 탭**에서는 헤더를 통째로 접고 얇은 툴바 한 줄만 남긴다. iframe 세로 공간을 약 110px 더 확보하기 위한 것이다 (일정·현황판이 잘려서 스크롤해야 하던 문제).
- 탭 이동은 왼쪽 **☰ 서랍**으로 한다. 헤더의 ☰, 사이트 툴바의 ☰ 둘 다 같은 서랍을 연다. 배경 클릭·Esc·항목 선택 시 닫힌다.
- 접속코드 안내와 「이 기기에서 잠그기」는 서랍 하단에도 들어 있다. 헤더가 접힌 상태에서도 코드를 확인할 수 있어야 하기 때문이다.
