...
## 6-2. 2차 입력 (2026-08-09, 후기 추가모집)

**원본 엑셀**: `…\2. 대학원\2026학년도 후기\★2026학년도 후기 대학원 2차 지원자 현황_정리본.xlsx` (시트 '대학원', 열배치 1차와 다름)
**차수: 2026 / 후기추가모집(sems=4) / 차수 2** — 일정학기관리에서 현재학기를 바꾸면 폼 기본값이 그대로 렌더됨(차수 함정 자동 해소, 그래도 `--chasu=2` 병기함).

- ✅ **11명 전원 저장 완료** (박사1·석사9·석사편입1, INSERT+UPDATE, `verify_all.js data2/*.json` 전건 일치)
  - 11번 응웬반밍(베트남·석사·미래융합기술공학과·TOPIK 4급)은 8/9 추가분 — DEPT에 미래융합, KR_SCHOOLS에 광주대학교 추가함
- 신형 파이프라인: `sheet2rows.ps1 -SheetXml xl/worksheets/sheet1.xml` → `gen_applicants2.js` → `data2/`
- DEPT 추가: 임상상담심리학과=보건상담정책(9)·이중언어O / 시각영상디자인→일반(4) '디자인학과'로 검색 (모집요강 9차 대조)
- **일정관리의 원서입력 종료일이 지나도 관리자 입력은 막히지 않음** (8/7 종료 → 8/9 정상 저장 실증)
- 석사편입 = gdhlDegCosCd 40 + gdhlEtshGbCd 2 (이모디, 석사 이력은 졸업(1)로 — 담당자 지시)
- 남은 일: ① 임시 등록번호 6명(가준희·장가녕·주충래·장화명·양가군·김주함·호효연 중 실번호 입수 시 `--load` 재저장)
  ② 가준희 TOPIK 3급 유효기간 공란 → 추후 보완 ③ 선소가 석사 졸업일 20260831은 "2026.8월 예정"의 월말 추정치 — 실제 일자 확인 시 수정
